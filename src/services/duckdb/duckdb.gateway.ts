...
export class DuckDBGateway extends GenericGateway {
  ...
  private createTable(): void {
    this.logger.info('Reach createTable() on DuckDBGateway');
    const sql = `
  ...
  private createSequence(): void {
    this.logger.info('Reach createSequence() on DuckDBGateway');
    const sql = `
  ...
  public getAll(): Promise<Array<Event>> {
    this.logger.info('Reach getAll() on DuckDBGateway');
  ...
  public get(id: number): Promise<Event> {
    this.logger.info(`Reach get(${id}) on DuckDBGateway`);
    return new Promise((resolve, reject) => {
  ...
  public create(event: Event): Promise<Event> {
    this.logger.info('Reach create() on DuckDBGateway');
    return new Promise((resolve, reject) => {
  ...
  public delete(id: number): Promise<number> {
    this.logger.info(`Reach delete(${id}) on DuckDBGateway`);
    return new Promise((resolve, reject) => {
